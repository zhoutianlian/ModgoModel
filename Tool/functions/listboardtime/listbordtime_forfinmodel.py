...
class GetListTime():
    ...
    def getNASDAQTime(self,equity, MV,re):
        ...
        if equity > equity_target1 * CNYtoUSD or (
                equity > equity_target2 * CNYtoUSD and (MV > MV_target * CNYtoUSD )):
            time = 0

        if time == 99:
            time = min(99,math.log((MV_target * CNYtoUSD  - MV) / MV + 1, 1 + re))
    ...
    def getHKATime(self, MV, re):
        ...
        time = 99
        MV_target = 100000000
        if MV > MV_target * CNYtoHKD:
            time = 0

        if time == 99:
            time = min(99,  math.log((MV_target * CNYtoHKD - MV) / MV + 1, 1 + re))
