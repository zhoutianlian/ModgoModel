...
from Model.financial_report_model.light_model import ForcastFR
...
from Tool.functions.read.get_something import get_FFR
from Tool.functions.read.read_data_for_businessmodel import read_data
from Tool.functions.read.read_data_for_light_val import get_data_for_simplemodel
from Tool.functions.save.save_data_for_light_val import save_data
...
from Tool.hypothesis.dlol_re_adjust import dlol_re_adjust
...
import Model.valuation_models.normal_valuation.summary as s
...
def value(vid, *args, **kwargs):
    ...
    """
    # 从数据库得到值
    [Dr, AP, debt, Cash, Inv, AR, TA, L, SC, NI, EBIT, EBT, Rev, indus_code, pc_A, pc_3, pc_pbp, pc_pep, pc_evsp, exitY,
     bestY, exitYpc,
     TVpc, perpetuity_g_basic, perpetuity_eva_pc, perpetuity_ri_pc, M_pc, perpetuity_g_pc, perpetuity_ed_pc, CS,
     financialreport_year, riskfree_rate, code_list, Rev_adjustment, flag, capital_market, finance_round
     ] = get_data_for_simplemodel(vid)

    # 财报预测
    Input = {'Indus_code': indus_code, 'TA': TA, 'Cash': Cash, 'AR': AR, 'Inv': Inv, 'debt': debt, 'Dr': Dr, 'AP': AP,
             'L': L, 'Rev': Rev, 'EBIT': EBIT, 'EBT': EBT, 'NI': NI, 'SC': SC, 'Rev_adj': Rev_adjustment}
    A = ForcastFR(Input)
    a = A.Output()
    ...
    #                 main_industry = e
    #                 break
    #     business_model_input = read_data(bm_id, flag)
    ...
    re, a = dlol_re_adjust(riskfree_rate, indus_code, capital_market, finance_round, a)
    # Wacc
    wacc = WACC(re, a, CS)
    # Samuelson 期权法
    try:
        SAM = VMF.choose_valuation_model(ValuationModels.NOR_SAM_LITE)
        sam_input_norlite = {'asset': TA, 'sales3': Rev[2], 'sales2': Rev[1], 'sales1': Rev[0], 'debt': debt,
                             'liabi': L,
    ...
    save_data(str(vid), abs_R, market_R, samuelson_result, SS_R, wacc, a, para, FFR, market_para, listtime_ans, gpscore,
