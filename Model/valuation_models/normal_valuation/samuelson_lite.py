...
class Samuelson_Normal_Lite():

    def valuation_samuelson(self,sam_input_forlite):
        # 暂定的变量
        # 无风险利率按照2018年最后一天取
        year_now = 2018
        ...
        # 分解输入值
        asset=sam_input_forlite['asset']
        sales3=sam_input_forlite['sales3']
        sales2=sam_input_forlite['sales2']
        sales1=sam_input_forlite['sales1']
        debt=sam_input_forlite['debt']
        liabi=sam_input_forlite['liabi']
        wacc=sam_input_forlite['wacc']
        interestrate=sam_input_forlite['interestrate']
        induscode=sam_input_forlite['induscode']
        SC=sam_input_forlite['SC']
        ...
        # db = connect_mysql_Require_Rate_of_Return()
        # cursor = db.cursor()
        # sql = "select GovermentsBond10 from Faye_GovernmentBond as f order by f.Date desc limit 1"
        # try:
        ...
        ga = np.average([math.log(sales2 / sales1), math.log(sales3 / sales2)]).item()
        ...
        # 计算N(d1)和N(d2)
        #  计算sigma
        salesrate1_2 = math.log(sales2 / sales1)
        salesrate2_3 = math.log(sales3 / sales2)
        sigma = np.std([salesrate1_2, salesrate2_3]).item()
        ...
        # 判断合法
        if result_legal('nor_sam_lite', dic_mv):
        ...
        # if dic_mv['MV_min'] < 0:
        # log1 = math.log(sales2 / sales1)
        # log2 = math.log(sales3 / sales2)
