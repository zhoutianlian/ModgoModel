...
from Tool.functions.distribution import for_distribution
from Tool.functions.for_summary.sub.decide_proportion import get_proportion
...
def summary_model(results,scal,show_in_page,SC):
    ...
    the_min=max([scal['min_mkt'],scal['min_ass'],scal['min_sam']])
    for e in [scal['min_mkt'],scal['min_ass'],scal['min_sam']]:
        if e!=0 and e<the_min:
            the_min=e
    the_max=max([scal['max_mkt'],scal['max_ass'],scal['max_sam']])
    while True:
                ...
                while mark[index_for_mark]<len(current_distri) and current_max>block_bottom and current_min<=block_bottom:
                    ...
                    if current>block_up:
                        ...
                        if len(ans_high)<index+1:
                            ...
                            logger(2, 'error in summary,mark3')
    ...
    for count in range(total_step):
        if ans_high[count] == target:
            ...
            if ans_max == 0:
                ans_max = the_max
    score_abs = math.pow(scal["score_ass"], 2)
