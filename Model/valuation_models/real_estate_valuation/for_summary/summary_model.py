...
def summary_model(results,scal,show_in_page,SC):
    ...
    the_min=max([scal['min_est'],scal['min_a'],scal['min_b']])
    for e in [scal['min_est'],scal['min_a'],scal['min_b']]:
        if e!=0 and e<the_min:
            the_min=e
    the_max=max([scal['max_est'],scal['max_a'],scal['max_b']])
    while True:
    ...
    summary_result = {'MV_max': ans_max, 'MV_min': ans_min, 'MV_avg': ans_avg, 'p_max': (ans_max / SC),
                      'p_min': (ans_min / SC), 'p_avg': (ans_avg / SC)}
